...
public class AdGydeTrackerPlugin extends CordovaPlugin 
{
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException 
	{
        ...
        // INIT PAgent
        if (action.equals("initPAgent") && args.length() == 2) 
		{
            return initPAgent(optString(args, 0), optString(args, 1));
        }
        ...
        if (action.equals("eventDailyUnique") && args.length() == 3) 
		{
            return  eventDailyUnique(optString(args,0), optMap(args,1),flags);
        }

        if (action.equals("eventPermanentUnique") && args.length() == 3) 
		{
            return  eventPermanentUnique(optString(args,0), optMap(args,1),flags);
        }
    ...
    private String getVersion() 
	{
        ...
        return "AdGyde SDK v4.0.3";
    }

    private boolean initPAgent(final String appKey, final String channel)
	{
        Log.i(LOGTAG, "initPAgent : appkey- " + appKey + " channel- " + channel);

        PAgent.init(context, appKey, channel); // Default channel is Organic
    ...
    private boolean setCurrentScreen(final String screenName) 
	{
        ...
        PAgent.setCurrentScreen(context, screenName);
    ...
    private boolean removeCurrentScreen(final String screenName) 
	{
        ...
        PAgent.removeCurrentScreen(context, "HomeScreen");
    ...
    private boolean eventSimple(final String eventName) 
	{
        ...
        PAgent.onSimpleEvent(eventName);
        return true;
    }
    ...
    private boolean eventCounting(final String eventName,  HashMap<String, String> value) 
	{
        ...
        PAgent.onCountingEvent(eventName, value);
    ...
    private boolean eventComputing(final String eventName,  HashMap<String, String> value) 
	{
        ...
        PAgent.onComputingEvent(eventName, value);
        return true;
    }

    private boolean eventDailyUnique(final String eventName, final HashMap<String, String> value,Boolean flag)
	{
        PAgent.onDailyUnique(eventName,value,flag);
        return true;
    }

    private boolean eventPermanentUnique(final String eventName, final HashMap<String, String> value,Boolean flag)
	{
        PAgent.onPermanentUnique(eventName,value,flag);
        return true;
    }

    private boolean eventCustomUnique(final String eventName, final HashMap<String, String> value, int time)
	{
        PAgent.onCustomUnique(eventName,value,time);
        return true;
    }

    private boolean eventRevenue(int rev)
	{
        PAgent.onRevenue(rev);
        return true;
    }
