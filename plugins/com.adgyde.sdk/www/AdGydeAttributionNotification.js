 /*************************************************************************
 *  File: AdGydeAttributionNotification.js
 *  Description: AdGyde Attribution Notifiation JavaScript File
 *
...
(
	function(cordova) 
	{
		...
		AttributionNotification.prototype.notificationCallback = 
			function(notification) 
			{
				console.log('Received Attribution Notification');
				var ev = document.createEvent('HTMLEvents');
				ev.notification = Base64.decode(notification);
				ev.initEvent(AdgydeTracker.ATTRIBUTION_EVENT_TYPE, true, true, arguments);
				document.dispatchEvent(ev);
			};

		// Event spawned when a consent status notification is received while the application is active
		ConsentStatusChangeNotification.prototype.notificationCallback = 
			function(notification) 
			{
				console.log('Received Consent Status Change Notification');
				var ev = document.createEvent('HTMLEvents');
				ev.initEvent(AdgydeTracker.CONSENT_STATUS_CHANGE_EVENT_TYPE, true, true, arguments);
				document.dispatchEvent(ev);
			};
		...
		cordova.addConstructor(
			function() 
			{
					console.log('Setup Adgyde Native Notifications');
					if(!window) window = {};
